...
namespace WeihanLi.Common.Models
{
    public static class ModelValidator
    {
        ...
        public static bool TryValidate(object instance, out IReadOnlyDictionary<string, string> result)
        {
            ...
            result = results.SelectMany(r => r.MemberNames.Select(m => new KeyValuePair<string, string>(m, r.ErrorMessage))).GroupBy(kv => kv.Key, kv => kv.Value).ToDictionary(kv => kv.Key, kv => kv.First());
            return false;
        }
