...
### [WeihanLi.Common 1.0.51](https://www.nuget.org/packages/WeihanLi.Common/1.0.51)

- Add `InMemoryStream` class to handle in-memory stream operations
- Update `EventQueueInMemory` to support stream-like operations using `InMemoryStream`
- Update `EventBus` to use `InMemoryStream` for stream operations
- Add tests for `InMemoryStream`
